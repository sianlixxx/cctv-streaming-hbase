...
class AppLogStreaming {

}
object AppLogStreaming extends Logging{

  def createContext(checkPointDir: String) = {
    val conf = new SparkConf().setAppName("cctv_streaming_app_log")
    //      .setMaster(spark)
    ...
    val queueName = conf.get("spark.cctv.amqp.queueName.app")
    val bindingKey = conf.get("spark.cctv.amqp.bindingKey.app")
    ...
    val hdfsPlayPath = conf.get("spark.cctv.hdfs.path.app")
